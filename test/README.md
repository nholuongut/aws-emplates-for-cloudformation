...
[nholuongut GmbH](https://nholuongut.net) sponsors the test runs on every push and once per week to ensure that everything is working as expected.
...
* `TEMPLATE_DIR` Load templates from local disk (instead of S3 bucket `nholuongut-aws-cf-templates`). Must end with an `/`. See `BUCKET_NAME` as well.
...
```
AWS_REGION="us-east-1" HOSTED_ZONE_ID="..." DOMAIN_SUFFIX="..." CLOUDFRONT_ACM_CERTIFICATE_ARN="..." ACM_CERTIFICATE_ARN="..." BUCKET_REGION="..." BUCKET_NAME="..." TEMPLATE_DIR="/path/to/nholuongut-aws-cf-templates/" mvn test
```
